...
case "$response" in
    [Yy][Ee][Ss]|[Yy])
        ...
        wget -q -nv -O - https://api.github.com/repos/shelevenspace/notshot/releases/latest | awk -F': ' '/browser_download_url/ && /notshot-guimodule-1\.[0-9]+\.[0-9]+\.tar\.gz/ {gsub(/"/, "", $(NF)); system("wget -qi -L " $(NF))}' || (echo "Error downloading latest release of gui module!" && exit 1) # download the latest version of notShot gui 1
