...
def capture(guiargs = None):
    ...
    # handle if ran from gui
    if guiargs is not None: # the gui passed its settings, so replace notShot's defaults / any from the command line
        arg.verbose = guiargs[0]
        arg.nostructure = guiargs[1]
        arg.seeimage = guiargs[2]
        arg.quiet = guiargs[3]
...
if __name__ == "__main__": # this is True if you're running notshot without the gui front-end
    capture()
